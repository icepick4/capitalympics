...
export default class ApiService {
    ...
    {
        lang ||= getLanguage();
        const response = await ApiClient.get<{ countries: CountryI[] }>('/countries', { lang, max })
        if (!response.success) {
    ...
    public static async getCountry(code: string, lang: Lang = 'en'): Promise<CountryI>
    {
        const response = await ApiClient.get<{ country: CountryI }>(`/countries/${code}`, { lang });
        if (!response.success) {
            throw new Error(`An error occurred while retrieving the country with the code '${code}'`);
        }
    ...
    public static async getUserScore(user: User): Promise<Level>
    {
        const response = await ApiClient.get<{ flag_score: Level, capital_score: Level }>(`/users/${user.id}/score`);
        if (!response.success) {
            throw new Error('Failed get global score');
        }

        const avg = (response.data.flag_score + response.data.capital_score) / 2;
