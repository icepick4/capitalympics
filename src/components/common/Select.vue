...
type Option = { value: OptionValue, label: Maybe<OptionValue>, disabled?: boolean } | OptionValue;
...
const selectedItem = computed(() => items.value.find(item => item.value === selectedItemValue.value));
...
<template>
    <div class="relative block w-full font-sans space-y-1">
        ...
        >
            <ListboxButton
                class="
                    w-full bg-white flex items-center py-2 px-3 gap-x-2 rounded-md text-left text-base
            ...
            >
                <slot name="prepend">
                    <div
                        ...
                        :class="[open || selectedItem ? 'text-black/75' : 'text-gray-300']"
                    >
                ...
                <div
                    aria-hidden="true"
                    class="grid place-items-center h-8 w-8 transition-colors shrink-0 text-gray-300"
            ...
            <ListboxOptions class="
                absolute z-20 origin-top-left mt-2.5 max-h-60 w-full min-w-[72px] overflow-auto
                rounded-md bg-white p-1 text-base sm:text-sm
                outline outline-2 outline-offset-2 outline-transparent focus-visible:outline-blue-600/75
            ">
                <ListboxOption
                    v-for="item in items"
